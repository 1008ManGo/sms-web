...
1. **国家权限**：用户被分配了目标国家
2. **通道权限**：用户被分配了至少一个发送通道
3. **账号状态**：用户账号处于Active状态
4. **余额充足**：账户余额足够支付短信费用

#### UserCountryPermission（用户国家权限）
...
```
1. 提取手机号国家代码
2. 检查用户是否有该国家权限 → 无 → 返回COUNTRY_NOT_ALLOWED
3. 检查用户是否有通道权限 → 无 → 返回NO_CHANNEL_ASSIGNED
4. 检查账号状态和余额 → 不足 → 返回BALANCE_INSUFFICIENT
5. 允许发送
```
